...
export function EmptyState({ onCreateBot, language }: EmptyStateProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full px-4">
      ...
      <h3 className="text-gray-900 mb-2 text-center text-base sm:text-lg">{t.title}</h3>
      <p className="text-gray-500 text-sm mb-4 text-center max-w-md">
        {t.description}
      </p>
      <Button onClick={onCreateBot} className="bg-blue-500 hover:bg-blue-600 text-white">
        {t.button}
      </Button>
    </div>
  );
